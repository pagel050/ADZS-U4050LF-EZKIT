...
*********************************************************************************/
/*!
 * @file      core_driven_crc.c
 * @brief     Example to demonstrate core-driven CRC operations for computing
 *            the CRC of given block of data.
 * @details
 *            This is the primary source file for the CRC example that shows
 *            how to use core-driven CRC operations for computing the CRC of
 *            given block of data.
 */
...
int main (void)
{
    ...
    else if (ADI_CRC_SUCCESS != adi_crc_Compute (hCrcDev, StartAddr, NumBytes, NumBits))
    {
        ...
        else if (ADI_CRC_SUCCESS != adi_crc_GetFinalCrcVal(hCrcDev, &computedCRCValue))
        {
